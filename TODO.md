...
- [x] 🔨 Create Python project structure + virtual environment
- [x] 🔨 Download ClinVar VCF file (public, ~100MB)
- [x] 🔨 Parse ClinVar: extract pathogenic + benign variants (chrom, position, ref allele, alt allele, label)
- [x] 🔨 Download human reference genome (GRCh38, FASTA) — or just chr17 + chr13 to start small
- [x] 🔨 Write `devlog.md` Day 1 entry
- [ ] 📖 Read: What is ClinVar? What is a VCF file? What is a reference genome? (`concepts.md` Section 1)

### Day 2 — Sequence Extraction
- [x] 🔨 For each variant, extract ~1000bp DNA window centered on the variant position from the reference genome
- [x] 🔨 Create two versions of each window: reference (normal) and alternate (with the mutation)
- [x] 🔨 Split into train/val/test (by chromosome to avoid data leakage — e.g., chr1-16 train, chr17 val, chr18-22 test)
- [x] 🔨 Save as a clean dataset (CSV or parquet with: chrom, pos, ref_seq, alt_seq, label)
- [x] 🔨 Verify: print some examples, confirm the variant is in the middle of the window
- [ ] 📖 Read: Why split by chromosome? What is data leakage? (`concepts.md` Section 2)

### Day 3 — Tokenization + Dataset
- [x] 🔨 Install `transformers` from HuggingFace
- [x] 🔨 Load NT-v2 tokenizer: `AutoTokenizer.from_pretrained("InstaDeepAI/nucleotide-transformer-v2-50m-multi-species")`
- [x] 🔨 Tokenize reference and alternate sequences
- [x] 🔨 Build a PyTorch `Dataset` class that returns (ref_tokens, alt_tokens, label)
- [x] 🔨 Build DataLoaders (batch size, shuffle, etc.)
- [x] 🔨 Verify: print one batch, confirm tensor shapes look right
- [ ] 📖 Read: What is a tokenizer? What are embeddings? What is a DataLoader? (`concepts.md` Section 3)

### Day 4 — Fine-Tuning
- [x] 🔨 Load pre-trained NT-v2: `AutoModel.from_pretrained(...)`
- [x] 🔨 Freeze the base model weights (we only train the classification head)
- [x] 🔨 Add a classification head on top: NT-v2 embeddings → linear layer → pathogenic/benign
- [x] 🔨 Design input strategy: encode ref and alt separately, subtract embeddings, classify the difference
- [x] 🔨 Write training loop (or use HuggingFace Trainer)
- [x] 🔨 Train on Colab T4 — should take 30-60 min
- [x] 🔨 Save the model checkpoint
...
- [x] 🔨 Implement integrated gradients (or use `captum` library)
- [x] 🔨 For a given variant prediction, compute attribution scores per base
